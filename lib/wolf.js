function wolf(){
    ...
    this.monitor = function(app, token, path){
        webot.set('subscribe', {
            ...
            handler: function(info) {
              ...
              +'\n2,第一次进入房间请严格按照标准："房间号 名字"输入，房间号与名字之间有一个空格，名字中不能有空格。上帝查看游戏角色时，会显示这个名字。（以后只需输入房间号）'
        ...
        webot.set('creat room',{
            ...
            handler: function(info){ //return the message you want to send back to user
                ...
                user_list[info.uid] = roomId; //The create automatic to become one of the player in the room
                return ret;
            }
        })
        ...
        // 接管消息请求
        webot.watch(app, { token: token, path: path });
    }
};
